...
void setup() {
    ...
    //size(800, 480, P2D);
    numneurons = (width*height)/768;
 
    frameRate(5000);

    n = new Neuron[numneurons];
...
class Synapse {
    ...
    Synapse(int _A, int _B) {
        A = _A;
        B = _B;
        weight = random(101,1100)/300.9;  // The weight affects the travel speed of the impulses.
    }
}
...
class Signal {
    ...
    void step() {
        ...
        // The closer to the neuron the signal gets, the slower it approaches.
        ...
        // If the signal is very close to the neuron, 
